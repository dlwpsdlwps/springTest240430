...
public class PagingHandler {
	//list 화면 하단에 페이지 조작부 기능을 담당하는 변수들을 생성
	//시작페이지 번호 / 끝페이지 번호 (한 페이지에서 보여지는 페이지네이션)
	//최종 끝번호
	//이전, 다음이 생성되는 여부
	...
	public PagingHandler(PagingVO pgvo, int totalCount) {
		...
		//최종 끝페이지
		//전체 게시글이 103라면 => 103/10 => 10.3(올림) => 11페이지
		this.realEndPage = (int)Math.ceil(totalCount/(double)pgvo.getQtt());
